classdef Test_AnalysisBuilder < TestCase 
	...
	methods 
        ...
        function test_writePlot(this)
            assert(~isempty(plotobj));
            this.director.writePlot(plotobj);
            assert(lexist(plotobj.filename));
        end
        function test_writeTable(this)
            assert(~isempty(tabobj));
            this.director.writeTable(tabobj);
            assert(lexist(tabobj.filename));
        end
        function test_sampleHarvardRegions(this)
            smpl = this.director.sampleHarvardRegions(this.harvardRois);
            assertEquals(this.harvardSamples, smpl);
        end
        function test_mcaCerebellarRatio(this)
        end
        function test_mcaPcaRatio(this)
        end
        function test_hemisphericRatio(this)
        end
        function test_sampleRegion(this)
            this.regions = this.director.harvardRegions;
            this.director.ensureCoregistered2Pet
            this.director.ensureCoregistered2Mr;
            smpls = this.director.sampleRegions(this.regions);
            assert(~isempty(smpls));
        end
